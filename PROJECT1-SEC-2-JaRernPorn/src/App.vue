...
//word data
const answer = ref('')
//check selectedAnswer if is checking or not
const isChecking = ref(false)
//user selected answer
const selectedAnswer = ref('')
//show options
const options = ref([])
//index of current item data
const currentIndexItem = ref(0)
//index of current category data
const currentIndexCate = ref(0)
//setting button
const settingButton = ref(false)
//arr of user answer by click on options
...
<template>
  ...
  <section class="homePage" v-if="allPage.homePage">
    <div class="h-screen w-full bg-main-bgColor p-5" :class="{ absolute: popUp.showHowToPlayPage }">
      <div class="border-double border-8 border-title box-border h-full w-full">
        ...
        <div class="text-black text-center">
          <p class="font-outfit font-medium tracking-sub-title sm:text-sub-title-size">
        ...
        <div id="home-page-button">
          ...
          <div id="play-button" class="flex justify-center pt-12">
            <img class="w-72 relative hover:w-80 transition-all duration-300 ease-in-out cursor-pointer"
              src="/home-page/play-button.svg" alt="play-button" @click="playButton" />
          </div>
          <!-- How to Play Button -->
          <div id="HTP-button" class="flex justify-center pt-6">
            <img class="w-72 relative hover:w-80 transition-all duration-300 ease-in-out cursor-pointer"
              src="/home-page/HTP-button.svg" alt="HTP-button" @click="howToPlayButton" />
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- How To Play Page -->
    <div class="flex justify-center items-center h-screen bg-black bg-opacity-50 absolute" v-if="popUp.showHowToPlayPage" @click.self="closeHowToPlay">
      <div
        ...
        <div class="sticky top-3 cursor-pointer w-10" @click="closeHowToPlay">
          <img class="w-6 sticky left-3" title="close" src="/HowToPlay-page/close-vector.png" alt="close-vector" />
        </div>
        <div class="flex justify-center items-center">
          <div class="w-3/4">
            <h1 class="text-5xl pt-4 font-bold text-center mb-6 font-alkatra text-title">
              HOW TO PLAY
            </h1>
            <div class="content">
              <div class="HTP-1">
              ...
              <div class="HTP-3">
                ...
                <img class="mb-9 w-screen rounded-lg" src="/HowToPlay-page/HTP-3.png" alt="HTP-3" />
              </div>
    ...
    <div class="p-7 bg-main-bgColor min-h-screen">
      <header>
        ...
        <img class="w-16 absolute hover:w-catePage-20 transition-all duration-300 ease-in-out cursor-pointer"
          src="/categories/icon/left-arrow.png" alt="left-arrow" @click="backToHome" />
      ...
      <div class="flex content-center justify-center mt-14">
        <div class="font-semibold text-black font-outfit md:flex md:justify-center">
          <div class="md:flex md:space-x-32 md:flex-wrap md:w-3/4 md:justify-center">
            <div v-for="(category, cateIndex) in categories" :key="category.name"
              class="category-item flex flex-col items-center md:mb-9 cursor-pointer" @click="showPlaygame(cateIndex)">
              <div class="pic w-52 pb-2 hover:w-56 transition-all duration-300 ease-in-out">
                <img :src="category.image" :alt="category.name" class="hover:drop-shadow-lg" />
              </div>
  ...
  <!-- Play Game Page -->
  <section class="playGame h-full w-full bg-main-bgColor" v-else-if="allPage.playgamePage">
    <div class="mx-auto h-full">
      <div class="header flex justify-evenly items-center py-6">
        <div class="play md:ml-4 text-center">
          <h1 class="text-4xl text-title lg:text-5xl font-outfit font-bold">
            Category: {{ currentCategory }}
          </h1>
        </div>

        <div class="setting flex items-center hover:scale-125 hover:transition-all duration-300 ease-in-out">
          <img src="/play-game/setting.png" alt="setting-image" class="w-9 lg:w-12 cursor-pointer" @click="openSetting" />
        </div>
      </div>

      <section id="settingButton" v-if="settingButton" class="fixed inset-0 z-50">
        <div class="w-full h-screen flex justify-center items-center bg-black bg-opacity-50">
          <div class="relative">
            <div class="rounded-3xl bg-white bg-opacity-100 w-[200px] h-[403px] sm:w-[300px] md:w-[450px] md:h-[403px]">
              <div class="flex-col p-5">
                <img src="/setting-features/cancel.png" alt="cancel button" class="w-5 sm:w-8 cursor-pointer"
                  @click="closeSetting" />

                <div class="text-center text-2xl font-alkatra text-title font-semibold sm:text-6xl"
                  style="text-shadow: 2px 2px 4px rgba(18, 17, 17, 0.5)">
                  Setting
                </div>

                <div class="flex flex-col items-center gap-4 mt-4">
                  <img src="/setting-features/resumeButton.png" alt="resume button"
                    class="resume button cursor-pointer hover:scale-110 hover:transition-all duration-300 ease-in-out"
                    @click="closeSetting" />
                  <img src="/setting-features/restartButton.png" alt="restart button "
                    class="mb-2 cursor-pointer hover:scale-110 hover:transition-all duration-300 ease-in-out"
                    @click="restartButton" />
                  <img src="/setting-features/homebutton.png" alt="home button"
      ...
      <div id="pic-frame" class="relative mx-auto w-[90%]">
        <img class="block mx-auto sm:my-8" src="/play-game/pic-frame.png" alt="frame for picture while playing game" />
        <img class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-1/2 sm:w-1/5 font-outfit"
          :src="currentQuiz" :alt="answer" />
      </div>

      <div class="flex flex-col justify-center items-center py-4 sm:py-10">
        <div class="grid grid-cols-1 md:grid-cols-2 md:grid-rows-2 gap-5 justify-center w-3/4 sm:w-3/5">
          <button v-for="(option, index) in options" :key="index" @click="() => {
            if (!isChecking) checkAnswer(option.value)
          }
            " :class="setButtonCorrect(option.value)"
            class="flex justify-center items-center bg-button-bgColor text-white font-normal text-2xl h-14 sm:h-20 rounded-md hover:drop-shadow-lg">
            {{ option.value }}
          </button>
        </div>
      </div>
  ...
  <!-- Wrong popup  -->
  <section v-if="popUp.showAnswer" class="fixed inset-0 flex justify-center items-center bg-black bg-opacity-75 z-50">
    <div class="w-full h-full flex justify-center items-center">
      <img src="/wrongPopup/bear-frame.png" alt="Frame chinese theme" class="mx-auto mobile:w-96 lg:w-3/5 xl:w-6/12" />
      <div
        ...
        <div
          ...
          The answer is<br />
          <div
            class="text-answer-fontColor font-NotoSansSC font-medium lowercase text-sm relative sm:text-2xl md:text-4xl mobile:text-[20px]">
            {{ answer }}
          </div>
        </div>
        <button
          class="text-sm text-brownColor font-semibold font-alkatra tracking-wider relative right-7 bottom-3 hover:text-answer-fontColor sm:right-[50px] sm:top-[18px] sm:text-2xl md:text-3xl lg:right-[60px] mobile:text-[20px]"
          @click="handleNextBtn">
          NEXT
        </button>
      </div>
    ...
    <div id="body" class="bg-main-bgColor w-full h-screen p-5 relative">
      <div
        ...
        <div class="text-center relative translate-middle text-white">
          <img src="/endgame-page/complimentFrame.png" alt="Picture that said Excellent!" class="mx-auto my-50" />
          <div class="absolute p-4 text-center transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
            <h1 class="uppercase text-4xl font-alkatra md:text-compliment-size sm:text-6xl xl:text-compliment-size">
              excellent!
            </h1>
            <p class="text-2xl font-outfit md:text-resultButton-size sm:text-4xl xl:text-resultButton-size">
  ...
  <section class="wordListPage" v-if="allPage.wordListPage">
    <div class="w-full min-h-screen bg-main-bgColor pt-4 flex justify-center">
      <img class="size-28 absolute left-5 md:size-36" src="/result-page/frame-top.png" alt="result-deco-top" />
      <div></div>
      <img class="size-28 absolute right-5 bottom-3 md:size-36" src="/result-page/frame-bottom.png"
      ...
      <div dir="rtl">
        <img
          ...
          src="/result-page/home button.png" alt="home" @click="backToHome" />
      </div>

      <div class="flex flex-col justify-center gap-4 md:gap-10">
        <div class="flex flex-col">
          <header class="font-alkatra font-semibold text-title text-7xl text-center md:text-8xl">
            Word List
          </header>

          <div class="font-outfit font-semibold text-md text-center tracking-subtitle sm:text-lg">
        ...
        <div class="flex flex-wrap justify-center gap-5">
          <section class="flex flex-col">
            <div
              ...
              <div v-for="(userAns, index) in userAnswer" :key="index" :class="{
                'text-red-500':
          ...
          <section class="flex flex-col">
            <div
              ...
              <div
                ...
                v-for="(answer, index) in categories[currentIndexCate].items" :key="index">
